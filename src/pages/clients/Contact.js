      ...
      <div className="py-16">
        ...
        <div className="flex w-[85%] mx-auto gap-5 py-10 px-0">
          {couses &&
            couses.map((cause) => (
              <div
                key={cause.id} // Add the key prop here
                className="text-center bg-gray-50 rounded-lg flex flex-col gap-3 py-10 px-5"
              >
                <h3 className="text-gray-900 font-semibold text-2xl">{cause.title}</h3>
                <p className="text-lg text-gray-700">{cause.desc}</p>
                <p className="text-sky-500 font-semibold text-xl">{cause.phn}</p>
              </div>
            ))}
        </div>
      </div>
